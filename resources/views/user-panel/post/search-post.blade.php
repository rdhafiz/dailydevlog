...
@section('content')
    ...
    <div id="searchBlogs" class="fixed-container mt-[50px]">

        <div class="mb-5 text-secondary dark:text-white block font-bold text-[16px] leading-[24px] dark:hover:text-second hover:text-second duration-500 text-truncate-line-2">Showing blogs matching the search for <span class="text-second">@{{ formData.keyword }}</span></div>
        ...
        <template v-if="tableData.length > 0 && !loading">
            <div class="grid grid-cols-1 sm:grid-cols-2 min-[917px]:grid-cols-3 gap-[15px]">
                <template v-for="(each, index) in tableData">
                    <div class="group rounded-2xl bg-white dark:bg-[#222222] shadow-lg sm:h-[316px]" :class="{ 'sm:col-span-2 block sm:grid sm:grid-cols-2 sm:gap-[15px]' : index === 0, 'col-span-1 lg:col-span-1 flex flex-col' : index > 0 }">
                        <a :href="`/blog-details/`+each.id" class="block rounded-2xl overflow-hidden min-h-[120px]" :class="{ 'sm:w-[316px] sm:h-[316px] sm:col-span-1 h-[120px] w-full' : index === 0, 'h-[125px]' : index > 0 }">
                        ...
                        <div class="grow flex flex-col gap-0 justify-between overflow-auto" :class="{ 'sm:col-span-1 text-center sm:text-left pr-[6px] pt-[15px] pb-2' : index === 0, 'px-2 pt-2 pb-1 text-center' : index > 0 }">
                            <div class="overflow-auto">
                                <a :href="`/blog-details/`+each.id" class="text-secondary dark:text-white block font-bold dark:hover:text-second hover:text-second duration-500" :class="{ 'text-truncate-line-3 text-[20px] leading-[30px]' : index === 0, 'text-truncate-line-2 text-[17px] leading-[25.5px]' : index > 0 }">
                                    @{{ each.title }}
                                </a>
                            ...
                            <div class="flex items-center font-[400] text-secondary dark:!text-light2 text-[13px] leading-[19.5px] pb-[2px] gap-1" :class="{ 'justify-center sm:justify-start' : index === 0, 'justify-center' : index > 0 }">
